...
def test_health_endpoint_returns_ok() -> None:
    ...
    assert 'broker_available' in payload
    ...
    assert 'api_key' not in payload['meeting_minutes_llm']
    assert 'key' not in payload['meeting_minutes_llm']
...
def test_upload_asset_saves_file_and_returns_asset_name() -> None:
    ...
    saved = (
        Path('F:/1work/音频识别/voiceprint-asr-platform/storage/uploads')
        / payload['asset_name']
    )
    assert saved.exists()
...
def test_jobs_endpoint_supports_delete() -> None:
    create = client.post(
        '/api/v1/transcriptions',
        json={'asset_name': 'sample-meeting.wav', 'diarization_model': None},
    )
...
def test_funasr_transcribe_real_wav_sample() -> None:
    ...
    if has_cuda_runtime():
        ...
        try:
            ...
            raise AssertionError('expected CUDA runtime error')
...
def test_downloaded_local_models_are_available_in_adapters() -> None:
    expected_cuda_availability = 'available' if has_cuda_runtime() else 'unavailable'
    assert (
        ThreeDSpeakerDiarizationAdapter(model_name='models/3D-Speaker/campplus').availability
        == expected_cuda_availability
    )
    assert (
        ThreeDSpeakerVoiceprintAdapter(model_name='models/3D-Speaker/campplus').availability
        == expected_cuda_availability
    )
    assert (
        PyannoteDiarizationAdapter(
            model_name='models/pyannote/speaker-diarization-community-1',
            enabled=True,
        ).availability
        == 'unavailable'
    )
...
def test_voiceprint_enroll_api_updates_profile() -> None:
    wav_path = Path(resolve_audio_asset_path('声纹-女1.wav'))
    created = client.post(
        '/api/v1/voiceprints/profiles',
        json={'display_name': '注册测试', 'model_key': '3dspeaker-embedding'},
    )
    ...
    response = client.post(
        f'/api/v1/voiceprints/profiles/{profile_id}/enroll',
...
def test_voiceprint_enroll_api_returns_404_for_unknown_profile() -> None:
    response = client.post(
        '/api/v1/voiceprints/profiles/unknown-profile/enroll',
        json={'asset_name': 'missing.wav'},
    )
...
def test_voiceprint_enroll_api_replace_mode_keeps_single_sample() -> None:
    ...
    created = client.post(
        '/api/v1/voiceprints/profiles',
        json={'display_name': '重复注册测试', 'model_key': '3dspeaker-embedding'},
    )
    profile_id = created.json()['profile']['profile_id']
    ...
    first = client.post(
        f'/api/v1/voiceprints/profiles/{profile_id}/enroll',
        json={'asset_name': upload_one.json()['asset_name']},
    )
    second = client.post(
        f'/api/v1/voiceprints/profiles/{profile_id}/enroll',
...
def test_voiceprint_verify_uses_requested_probe_asset() -> None:
    if has_cuda_runtime():
        ...
        try:
            voiceprint_service.verify(
                'sample-female-1',
                probe_asset_name='5分钟.wav',
                threshold=0.7,
            )
            raise AssertionError('expected unavailable high precision verification')
...
def test_voiceprint_identify_uses_requested_probe_asset() -> None:
    if has_cuda_runtime():
        ...
        assert all(
            candidate.profile_id in {'sample-female-1', 'demo-user'}
            for candidate in result.candidates
        )
    else:
        try:
            voiceprint_service.identify(probe_asset_name='5分钟.wav', top_k=2)
            raise AssertionError('expected unavailable high precision identification')
        except RuntimeError as exc:
            assert 'CUDA GPU' in str(exc)
...
def test_openapi_schema_has_chinese_summaries() -> None:
    ...
    assert paths_with_summary == paths_total, (
        f"接口中文 summary 覆盖率: {paths_with_summary}/{paths_total}"
    )
