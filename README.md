...
当前第一版指标包括 CER、文本相似度、热词召回、speaker 短碎片率、换人频率、声纹低置信统计、声纹 Top1/TopK 命中率、近似 EER 和会议纪要证据覆盖率。会议纪要诊断不仅输出决策、行动项、风险的覆盖率，还会为每条纪要生成 `evidence_rows`，记录证据分数、命中的 transcript 片段、弱证据项和缺证据项，避免长会议摘要只看总分而无法定位漏召回。
