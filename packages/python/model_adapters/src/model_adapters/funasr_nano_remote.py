...
class FunASRNano(nn.Module):
        ...
        if llm_conf.get("use_lora", False):
            ...
            if lora_init_param_path is not None:
                ...
                for name, param in model.named_parameters():
                    if not lora_conf.get("freeze_lora", False) and "lora_" in name:
    ...
    def forward(
        ...
        **kwargs,
        ...
        stats["dialog_turns_max"] = torch.max(dialog_turns).int().item()
        stats["dialog_turns_avg"] = dialog_turns.sum().item() / batch_size

        if self.length_normalized_loss:
            batch_size = int((labels_ids > 1).sum())
    ...
    def data_load_speech(self, contents: dict, tokenizer, frontend, meta_data=None, **kwargs):
        ...
        for i, (system_prompt, user_prompt, target_out) in enumerate(
            ...
            if i == 0:
                ...
                source_input = f"<|im_start|>user\n{user_prompt}"
                if not kwargs.get("infer_with_assistant_input", False):
                    source_input = f"<|im_start|>user\n{user_prompt}<|im_end|>\n<|im_start|>assistant\n"
            if not do_think:
                source_input += "<think>\n\n</think>\n\n"
                ...
                if not sub_str.startswith("<|startofspeech|>"):
                    ...
                    if sub_str.startswith("!"):
                        ...
                        try:
                            ...
                            logging.error(
                                f"Loading wav failed! {str(exc)}, {traceback.format_exc()}"
                            )
                            raise
                        ...
                        if self.use_low_frame_rate:
                            olens = 1 + (speech_lengths[0].item() - 3 + 2) // 2
                            olens = 1 + (olens - 3 + 2) // 2
    ...
    def inference_llm(self, data_in, data_lengths=None, key: list = None, tokenizer=None, frontend=None, **kwargs):
